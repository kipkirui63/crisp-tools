...
import { Upload, Home, Wand2, X, ChevronDown, Sparkles, Loader2, Download, AlertCircle } from 'lucide-react';
...
export default function AIInteriorDesign({ isAuthenticated, onRequestAuth }: InteriorDesignProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      formData.append('toolType', 'interior-design');
      ...
      setError(error.message || 'Failed to generate interior design');
    } finally {
        ...
        {generatedImages.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {generatedImages.map((image) => (
              <div
                key={image.id}
                className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-2xl overflow-hidden hover:border-cyan-500 transition-all group"
              >
