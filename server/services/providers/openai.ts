...
export class OpenAIProvider implements APIProvider {
  ...
  private async editImage(request: ImageGenerationRequest): Promise<ImageGenerationResponse> {
    ...
    const imageFile = new File([request.inputImage!], 'image.png', { type: 'image/png' });

    let maskFile: File | undefined;
    if (request.maskImage) {
      maskFile = new File([request.maskImage], 'mask.png', { type: 'image/png' });
    }

    console.log('[OpenAI] Calling images.edit with image file');
